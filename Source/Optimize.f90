...
!
!>  \par Updates
!>  \arg 27.11.2017 : new version of SteepLocator that use step of
!>                    conjugate gradient (optionally scaled down to
!>                    get real decrease of the target function)
!>
!******************************************************************************
...
!**************************************************************************************
   FUNCTION SteepLocator( GetPotAndForces, StartX, NMaxIter, GradThresh, SmallDelta ) RESULT( StationaryPoint )
      ...
      REAL, DIMENSION(size(StartX))              :: CurrentX, OldX
      REAL, DIMENSION(size(StartX),size(StartX)) :: EigenVectors, Hessian
      REAL, DIMENSION(size(StartX))              :: Forces, EigenValues, OldForces
      REAL :: V, GradNorm, OldV, Lambda
      INTEGER :: NIter
      ...
      Iterations: DO NIter = 1, NMaxIter
         ...
         DO
            ...
            ! Compute potential and forces at current position
            V = GetPotAndForces( CurrentX, Forces )
         ...
         IF ( GradNorm < GradThresh .OR. Lambda < 1.E-8 ) THEN
            EXIT Iterations
         END IF
